...
- **Keyboard shortcuts** (all off while a dialog is open): `Esc` closes the search (like its x); `Cmd`/`Ctrl`+`K` focuses the search box; `Cmd`/`Ctrl`+`A` selects every loaded message in the folder list for bulk actions (outside text fields, where it keeps its normal meaning; not in search or combined lists); `Cmd`/`Ctrl`+`R` replies to the open message instead of reloading the page (with no message open the browser's reload is untouched); `Backspace`/`Delete` deletes the open message or the bulk selection.
