...
The app opens on the combined Inbox (all accounts' Inboxes); pick a folder in the sidebar to see one account's mail.
...
- **Keyboard shortcuts** (all off while a dialog is open): `↑`/`↓` show the previous/next message in the active list (folder list, search results or combined lists) and `Shift`+`↑`/`↓` extends the bulk selection in a folder list — only with a mouse-type pointer (`pointer: fine`) and outside text fields and open menus; `Esc` closes the search (like its x); `Cmd`/`Ctrl`+`K` focuses the search box; `Cmd`/`Ctrl`+`A` selects every loaded message in the folder list for bulk actions (outside text fields, where it keeps its normal meaning; not in search or combined lists); `Cmd`/`Ctrl`+`R` replies to the open message instead of reloading the page (with no message open the browser's reload is untouched); `Backspace`/`Delete` deletes the open message or the bulk selection.
