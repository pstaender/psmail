...
- A password change (in Settings → Credentials) removes the stored password, since it would be stale; the same happens if a saved password is rejected at sign-in. Settings → Credentials shows whether it is set up on this device and can remove it. Deleting the passkey from the device makes the stored blob permanently unreadable — you just type the password again.
