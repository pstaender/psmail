...
- **Message list** — per-folder, with unread/flag indicators, attachment marker, and a snippet. Cmd/Ctrl+click to toggle individual messages, or Shift+click to select the whole range from the last plain/Ctrl-clicked message, for bulk Mark as read/unread, Move, or Delete (with confirmation); a plain click reads a message as usual and clears the multi-selection.
